...
ARTICLES_PER_HUB = 7   
HUBS_PER_HUB = 7       
OUTPUT_FILENAME = "ideal_hierarchy_v12.html"
    ...
    while article_count < TOTAL_ARTICLES and queue:
        ...
        for _ in range(to_create):
            ...
            nodes.append({
                "id": article_id,
                "group": "article",
                "depth": current_depth, # Technically same depth as parent for layout
...
<head>
    <meta charset="UTF-8">
    <title>Architect Console v12 (Geometric Core)</title>
    <style>
        :root {{
            --bg-color: #050505;
            --text-color: #ccc;
...
<body class="dark-mode">
    <div id="controls">
        ...
        <div id="status">Applying Geometric Seed...</div>
    </div>
    ...
    <script>
        ...
        // --- V12: RECURSIVE POLAR PARTITION ---
        // Instead of D3 Cluster, we manually calculate "Pie Slices".
        // Root gets 0 to 2PI.
        // Level 1 gets (2PI / N) each.
        // This guarantees a perfect 360 distribution.

        const stratify = d3.stratify()
            .id(d => d.id)
            .parentId(d => d.parentId);

        function applyGeometricSeed() {{
            try {{
                const root = stratify(rawGraph.nodes);
                
                // Recursive function to assign slice angles
                function assignAngles(node, startAngle, endAngle) {{
                    const totalSlice = endAngle - startAngle;
                    
                    // Place this node in the CENTER of its assigned slice
                    const myAngle = startAngle + (totalSlice / 2);
                    
                    // Radius depends on depth (manual spacing)
                    // We pull it in tight initially so it explodes outward
                    const myRadius = node.depth * 150; 
                    
                    // Map to Cartesian
                    // Rotate -90 (Math.PI/2) to start at 12 o'clock
                    const finalAngle = myAngle - Math.PI / 2;
                    
                    node.x = width/2 + myRadius * Math.cos(finalAngle);
                    node.y = height/2 + myRadius * Math.sin(finalAngle);

                    // Assign slices to children
                    if (node.children) {{
                        const step = totalSlice / node.children.length;
                ...
                // Start the recursion: Root gets full circle (0 to 2PI)
                assignAngles(root, 0, 2 * Math.PI);

                // Apply these coordinates back to the rawGraph nodes
                // D3 Force needs them on the flat node objects
                const nodeMap = new Map(root.descendants().map(d => [d.id, d]));
                
                rawGraph.nodes.forEach(n => {{
                    const geoNode = nodeMap.get(n.id);
                    if (geoNode) {{
                        n.x = geoNode.x;
                        n.y = geoNode.y;
                        
                        // STRICT PINNING FOR ROOT
                        if (n.group === 'root') {{
                            n.fx = width / 2;
                            n.fy = height / 2;
                        }}
                ...
                console.log("Geometric Seeding Complete.");
