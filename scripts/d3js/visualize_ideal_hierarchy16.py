...
ARTICLES_PER_HUB = 7   
HUBS_PER_HUB = 7       
OUTPUT_FILENAME = "ideal_hierarchy_v15.html"
...
<head>
    <meta charset="UTF-8">
    <title>Architect Console v15 (Intelligent Swarm)</title>
    <style>
        :root {{
            --bg-color: #050505;
            --text-color: #ccc;
...
<body class="dark-mode">
    <div id="controls">
        ...
        <div id="status">Applying Intelligent Swarm...</div>
    </div>
    ...
    <script>
        ...
        // --- V15: THE INTELLIGENT SWARM ---
        // 1. Hubs are calculated and LOCKED (fx, fy).
        // 2. Articles are calculated and PLACED (x, y) but NOT locked.
        // This prevents the "Hat" because articles start inside the castle walls.

        try {{
            const root = stratify(rawGraph.nodes);
            
            function assignPositions(node, startAngle, endAngle) {{
                ...
                if (node.data.group === 'root' || node.data.group === 'hub') {{
                    ...
                    // Articles get PLACED (Seeded), but NOT LOCKED
                    // This creates the "Swarm" effect without the "Hat" pop
                    node.data.x = calcX;
                    node.data.y = calcY;
                    // Note: No fx/fy for articles
                }}

                if (node.children) {{
                    // We distribute the angle slice equally among ALL children (Hubs AND Articles)
                    // This ensures articles actually fit into the pie slices allocated for them
                    const step = totalSlice / node.children.length;
                    node.children.forEach((child, i) => {{
                        const childStart = startAngle + (i * step);
                        const childEnd = startAngle + ((i + 1) * step);
                        assignPositions(child, childStart, childEnd);
                    }});
                }}
            }}

            assignPositions(root, 0, 2 * Math.PI);
            console.log("Swarm Seeded.");
        ...
        const simulation = d3.forceSimulation(rawGraph.nodes)
            ...
            // Radial force helps keep the free-floating articles from drifting too far
        ...
        function applyInitialZoom() {{
            ...
            document.getElementById("status").innerText = "Swarm Active.";
        }}
