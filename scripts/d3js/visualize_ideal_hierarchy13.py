...
ARTICLES_PER_HUB = 7   
HUBS_PER_HUB = 7       
OUTPUT_FILENAME = "ideal_hierarchy_v13.html"
...
def generate_ideal_graph():
    ...
    while article_count < TOTAL_ARTICLES and queue:
        ...
        if article_count < TOTAL_ARTICLES:
            # Check if we are at root to capture Level 1 hubs
            is_root_processing = (current_hub_id == root_id)
            ...
            for i in range(HUBS_PER_HUB):
                ...
                if is_root_processing:
                    level_1_hubs.append(new_hub_id)

                queue.append((new_hub_id, current_depth + 1))
                hub_count += 1

    # --- V13: STITCH THE RIM ---
    # Connect all Level 1 Hubs in a closed circle. 
    # This prevents the "Pac-Man" split by structurally forcing a ring.
    if len(level_1_hubs) > 1:
        for i in range(len(level_1_hubs)):
            source = level_1_hubs[i]
            target = level_1_hubs[(i + 1) % len(level_1_hubs)] # Wrap around to 0
            links.append({
                "source": source,
                "target": target,
                "type": "rim_link" # Special type for physics handling
            })
...
<head>
    <meta charset="UTF-8">
    <title>Architect Console v13 (The Iron Rim)</title>
    <style>
        :root {{
            --bg-color: #050505;
            --text-color: #ccc;
...
<body class="dark-mode">
    <div id="controls">
        ...
        <div id="status">Forging Iron Rim...</div>
    </div>
    ...
    <script>
        ...
        // --- V13: STRICT TRIGONOMETRY INITIALIZER ---
        try {{
            // 1. PIN THE ROOT
            ...
            // 2. PERFECT CIRCLE FOR LEVEL 1 (The Spokes)
            // We ignore the tree layout for these and force them into a circle.
            const spokes = rawGraph.nodes.filter(n => n.depth === 1 && n.group === 'hub');
            const angleStep = (2 * Math.PI) / spokes.length;
            const spokeRadius = 200; // Start them a bit out

            spokes.forEach((node, i) => {{
                // -Math.PI/2 to align first node to top (12 o'clock)
                const angle = (i * angleStep) - (Math.PI / 2); 
                node.x = width/2 + Math.cos(angle) * spokeRadius;
                node.y = height/2 + Math.sin(angle) * spokeRadius;
            }});

            // 3. TREE LAYOUT FOR THE REST
            // We use D3 cluster for the rest, but we are careful not to overwrite the root/spokes
            // This is just to give a decent starting point for the deep nodes.
            // (Physics will quickly take over anyway)
        ...
        const simulation = d3.forceSimulation(rawGraph.nodes)
            .force("link", d3.forceLink(rawGraph.links).id(d => d.id)
                .distance(d => {{
                    // Calculate chord length for the rim to keep it circular
                    if (d.type === 'rim_link') return 200; // Approx neighbor distance
                    return d.type === 'hub_link' ? 150 : 30;
                }})
                .strength(d => {{
                    // V13: IRON RIM STRENGTH
                    // Make rim links VERY strong to stitch the circle shut
                    if (d.type === 'rim_link') return 2.0; 
        ...
        const link = g.append("g")
            ...
            .attr("stroke-opacity", d => {{
                if (d.type === 'rim_link') return 0; // Completely invisible
                return 1.0;
            }});
