...
ARTICLES_PER_HUB = 7   
HUBS_PER_HUB = 7       
OUTPUT_FILENAME = "ideal_hierarchy_v10.html"
    ...
    while article_count < TOTAL_ARTICLES and queue:
        ...
        for _ in range(to_create):
            ...
            nodes.append({
                ...
                "heat": heat,
                "val": 7,
                "parentId": current_hub_id # V10: Explicit Parent ID for Tree Calc
            })
        ...
        if article_count < TOTAL_ARTICLES:
            for i in range(HUBS_PER_HUB):
                ...
                nodes.append({
                    ...
                    "label": "Cat",
                    "val": hub_size,
                    "parentId": current_hub_id # V10: Explicit Parent ID
                })
...
<head>
    <meta charset="UTF-8">
    <title>Architect Console v10 (Uncrossed)</title>
    <style>
        :root {{
            --bg-color: #050505;
            --text-color: #ccc;
...
<body class="dark-mode">
    <div id="controls">
        ...
        <div id="status">Initializing Smart Seed...</div>
    </div>
    ...
    <script>
        ...
        // --- V10: TOPOLOGICAL SEEDING ---
        // We use d3.stratify and d3.tree to calculate the PERFECT initial non-overlapping positions.
        // This untangles the hairball before physics even starts.
        ...
        try {{
            ...
            // Layout the tree radially to find ideal angles
            // We use a huge radius just to get the angular separation right
            const treeLayout = d3.cluster().size([2 * Math.PI, 2000]); 
            treeLayout(root);

            // Map the calculated tree coordinates back to our flat node list
            // to serve as the starting point (x, y) for the physics simulation.
            ...
            rawGraph.nodes.forEach(node => {{
                ...
                if (treeNode) {{
                    ...
                    // We center it at width/2, height/2
                    const theta = treeNode.x - Math.PI / 2; // Rotate -90 deg to start top
                    const r = treeNode.y; // This r is just for initial sorting, physics will override
                    
                    // Initial Position
                    node.x = width/2 + r * Math.cos(theta) * 0.1; // *0.1 compresses it so it explodes outward
                    node.y = height/2 + r * Math.sin(theta) * 0.1;
                }}
            }});
            console.log("Topological Seeding Complete.");
        }} catch (e) {{
            console.warn("Seeding failed (graph might not be a strict tree):", e);
        }}
        ...
        function updateColors() {{
            ...
            if (isLight) {{
                ...
                // Boosted luminance for Dark Mode: 10 to 180 range
        ...
        // --- INITIAL ZOOM (Locked to 0.19) ---
        function applyInitialZoom() {{
            // We use the computed center of the graph to translate correctly
