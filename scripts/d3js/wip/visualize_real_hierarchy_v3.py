...
# --- CONFIGURATION ---
# Adjust this path if necessary to point to your actual _context folder
# Based on your file tree, this seems to be the spot:
CONTEXT_DIR = Path.home() / "repos/MikeLev.in/_posts/_context"
OUTPUT_FILENAME = "real_hierarchy_viz.html"

# "Rule of 7" Constraints
MAX_HUBS = 7
MAX_ARTICLES_PER_HUB = 30 # Loosened slightly for real data visibility
...
def build_graph_from_shards(shards):
    ...
    all_keywords = []
    for shard in shards:
        # Normalize to lowercase to group 'Python' and 'python'
        ...
        for kw in top_keywords:
            if kw in [k.lower() for k in shard.get('kw', [])]:
                ...
                if hub_counts[kw] < MAX_ARTICLES_PER_HUB:
                    ...
                    # Simulated Heat (can be refined later with GSC data)
                    heat = 0.8 
