...
7.  **THE FOUR-BACKTICK RULE:** When generating SEARCH/REPLACE blocks, wrap each block in a four-backtick outer fence. Never use three-backtick fences as the outermost container for a SEARCH/REPLACE block. This prevents Markdown renderers from treating Python comments as headers and prevents backticks inside Python strings from collapsing the fence prematurely.
8.  **THE TARGET ADJACENCY RULE:** Every `[[[SEARCH]]]` marker must be immediately preceded by `Target: \`filename\`` on the line directly above it — no blank lines, no fences, no prose between the Target line and the marker. The filename in the Target line is what `apply.py` uses to find the file; omitting it or separating it with blank lines causes a fatal "Missing target filename" error. Use relative paths (`scripts/articles/lsa.py`) or absolute paths — both work.
'''
