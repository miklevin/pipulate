...
# - EARMARK: TURN SHAPE / THE PATCH TRAIN (banked 2026-07-18): response order is Probes -> Context -> Patches -> Prompt -> External. The prompt.md text rides LAST — the CABOOSE COPY — so it sits under the operator's cursor when the train stops. Probe block and adhoc echo block are copy-symmetric: identical commands, the echo adds only the leading "! ". Actuation choreography is the train itself: patch, app, d, m per car; blast (or git push) as the caboose. OWES: amend rule 11 in prompt_foo.py to this order and the Caboose Copy name (needs prompt_foo.py in context), then delete this line.
...
# apps/830_pico_slider.py  # [4,361 tokens | 19,302 bytes]
