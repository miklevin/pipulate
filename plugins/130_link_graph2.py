...
class LinkGraph2:
    ...
    **DO NOT REFACTOR THIS PATTERN WITHOUT UNDERSTANDING IT COMPLETELY**
    ...
    TEMPLATE_CONFIG = {
        ...
        'crawler': 'Crawler Basic',  # New basic crawl for node metadata
    ...
    def __init__(self, app, pipulate, pipeline, db, app_name=APP_NAME):
        ...
        crawler_template = self.get_configured_template('crawler')  # ← New template reference00
        ...
        steps = [
            ...
            Step(id='step_crawler', done='crawler_data', show='Download Crawl: Basic', refill=False,),
...
"""
    ...
    async def step_analysis_process(self, request, step_context=None):
        ...
        if step_context == 'step_crawler':
            active_edges_template_key = self.get_configured_template('crawler')
        else:
            active_edges_template_key = self.get_configured_template('edges')
        active_template_details = self.QUERY_TEMPLATES.get(active_edges_template_key, {})
        export_type = active_template_details.get('export_type', 'crawl_attributes')
        ...
        try:
            ...
            if file_exists:
                ...
                collection = f'crawl.{analysis_slug}'
                if step_context == 'step_crawler':
                    edges_template = self.get_configured_template('crawler')
                else:
                    edges_template = self.get_configured_template('edges')
                template_query = self.apply_template(edges_template, collection)
