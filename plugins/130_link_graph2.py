...
class LinkGraphVisualizer2:
    ...
    DISPLAY_NAME = 'Link Graph Visualizer 2 🔗'
    ...
    async def step_02(self, request):
        ...
        try:
            ...
            if selected_analysis:
                # For initial load, check for auto-optimized file (no depth suffix)
            ...
            # Only show depth selection for Link Graph Edges template
            depth_controls = []
            if active_crawl_template_key == 'Link Graph Edges':
                depth_controls = [
                    P("Depth Control:", style='margin-top: 15px; margin-bottom: 5px; font-weight: bold;'),
                    Select(
                        Option("Auto-optimize depth (~1M edges)", value="auto", selected=True),
                        Option("Depth 1", value="1"),
                        Option("Depth 2", value="2"),
                        Option("Depth 3", value="3"),
                        Option("Depth 4", value="4"),
                        Option("Depth 5", value="5"),
                        Option("Depth 6", value="6"),
                        Option("Depth 7", value="7"),
                        Option("Depth 8", value="8"),
                        Option("Depth 9", value="9"),
                        Option("Depth 10", value="10"),
                        name='hardwired_depth',
                        hx_post=f'/{app_name}/update_button_text',
                        hx_target='#submit-button',
                        hx_swap='outerHTML',
                        hx_include="closest form"
                    ),
                    P("Auto-optimize finds the optimal depth for ~1M edges. Manual depths 1-10 give you direct control over crawl depth.", 
                      cls='text-muted', style='font-size: 0.9em; margin-top: 5px;')
                ]
    ...
    async def step_02_submit(self, request):
        ...
        hardwired_depth = form.get('hardwired_depth', 'auto').strip()
        if not analysis_slug:
            return P('Error: No analysis selected', style=pip.get_style('error'))
        
        # Log depth selection
        if hardwired_depth != 'auto':
            await self.message_queue.add(pip, f'📊 Selected analysis: {analysis_slug}. Using hardwired depth: {hardwired_depth}. Starting crawl data download...', verbatim=True)
        else:
        ...
        analysis_result = {
            ...
            'export_type': active_template_details.get('export_type', 'crawl_attributes'),
            'hardwired_depth': hardwired_depth if hardwired_depth != 'auto' else None
        }

        # Execute qualifier logic if enabled AND no hardwired depth is specified
        if qualifier_config.get('enabled') and hardwired_depth == 'auto':
            try:
                api_token = self.read_api_token()
                if not api_token:
                    return P('Error: Botify API token not found. Please connect with Botify first.', style=pip.get_style('error'))
        ...
        elif hardwired_depth != 'auto':
            # User specified a hardwired depth - use it directly
            analysis_result['dynamic_parameter_value'] = int(hardwired_depth)
    ...
    async def get_deterministic_filepath(self, username, project_name, analysis_slug, data_type=None, depth_suffix=None):
        ...
        Args:
            ...
            depth_suffix: Optional depth suffix for filename (e.g., "_depth3")
        ...
        # Add depth suffix to filename if specified
        if depth_suffix:
            name, ext = filename.rsplit('.', 1)
            filename = f'{name}{depth_suffix}.{ext}'
    ...
    async def check_cached_file_for_button_text(self, username, project_name, analysis_slug, data_type, depth_suffix=None):
        """Check if a file exists for the given parameters and return appropriate button text."""
        try:
            filepath = await self.get_deterministic_filepath(username, project_name, analysis_slug, data_type, depth_suffix)
...
"""
    ...
    async def step_02_process(self, request):
        ...
        hardwired_depth = analysis_result.get('hardwired_depth')
        
        # Create depth suffix for filename if hardwired depth is specified
        depth_suffix = None
        if hardwired_depth and export_type == 'link_graph_edges':
            depth_suffix = f'_depth{hardwired_depth}'
        
        try:
            crawl_filepath = await self.get_deterministic_filepath(username, project_name, analysis_slug, export_type, depth_suffix)
            file_exists, file_info = await self.check_file_exists(crawl_filepath)
            if file_exists:
...
'''
    ...
    async def update_button_text(self, request):
        """Update button text dynamically based on selected analysis and depth."""
        try:
            form = await request.form()
            analysis_slug = form.get('analysis_slug', '').strip()
            username = form.get('username', '').strip()
            ...
            # Create depth suffix for filename if hardwired depth is specified
            depth_suffix = None
            if hardwired_depth != 'auto' and export_type == 'link_graph_edges':
                depth_suffix = f'_depth{hardwired_depth}'
            
            # Check if files are cached for the selected analysis
            is_cached = await self.check_cached_file_for_button_text(username, project_name, analysis_slug, export_type, depth_suffix)
