...
class RadioButtonWorkflow:
        ...
        try:
            ...
            for group, group_options in grouped_options.items():
                ...
                for option in group_options:
                    radio = Label(Input(type='radio', name=step.done, value=option['value'], checked=option['value'] == selected_value, title=option['description'] if option['description'] else None), f" {option['label']}")
                    group_radios.append(radio)
                radio_groups.append(Fieldset(Legend(group), *group_radios))
            return Div(Card(H3(f'{step.show}'), Form(*radio_groups, Button('Submit', type='submit', cls='primary'), hx_post=f'/{app_name}/{step_id}_submit', hx_target=f'#{step_id}')), Div(id=next_step_id), id=step_id)
    ...
    async def step_01_submit(self, request):
        ...
        return Div(self.pipulate.revert_control(step_id=step_id, app_name=app_name, message=f'{step.show}: {value}', steps=steps), Div(id=next_step_id, hx_get=f'/{app_name}/{next_step_id}', hx_trigger='load'), id=step_id)