...
class CheckboxWorkflow:
    ...
    async def step_01(self, request):
        ...
        if 'finalized' in finalize_data and selected_values:
            return Div(Card(H3(f'🔒 {step.show}'), P('Selected options:', cls='fw-bold'), Ul(*[Li(value) for value in selected_values])), Div(id=next_step_id, hx_get=f'/{app_name}/{next_step_id}', hx_trigger='load'), id=step_id)
        elif selected_values and state.get('_revert_target') != step_id:
            return Div(pip.revert_control(step_id=step_id, app_name=app_name, message=f"{step.show}: {', '.join(selected_values)}", steps=steps), Div(id=next_step_id, hx_get=f'/{app_name}/{next_step_id}', hx_trigger='load'), id=step_id)
        ...
        try:
            ...
            for group, group_options in grouped_options.items():
                ...
                for option in group_options:
                    checkbox = Label(Input(type='checkbox', name=step.done, value=option['value'], checked=option['value'] in selected_values, title=option['description'] if option['description'] else None), f" {option['label']}")
                    group_checkboxes.append(checkbox)
                checkbox_groups.append(Fieldset(Legend(group), *group_checkboxes))
            return Div(Card(H3(f'{step.show}'), Form(*checkbox_groups, Button('Submit', type='submit', cls='primary'), hx_post=f'/{app_name}/{step_id}_submit', hx_target=f'#{step_id}')), Div(id=next_step_id), id=step_id)
    ...
    async def step_01_submit(self, request):
        ...
        return Div(self.pipulate.revert_control(step_id=step_id, app_name=app_name, message=f"{step.show}: {', '.join(values)}", steps=steps), Div(id=next_step_id, hx_get=f'/{app_name}/{next_step_id}', hx_trigger='load'), id=step_id)
