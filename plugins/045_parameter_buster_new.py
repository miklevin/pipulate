...
import matplotlib # Added for a potential backend setting
...
# Explicitly set a non-interactive backend for Matplotlib
matplotlib.use('Agg')
...
class ParameterBusterNewWorkflow:
    """
    ...
    """
    APP_NAME = "param_buster_new_internal"
    DISPLAY_NAME = "Parameter Buster New"
    ENDPOINT_MESSAGE = "Analyze URL parameters from Botify data (Crawl, Web Logs, GSC) with enhanced UI and debugging features. Identifies optimization opportunities."
    TRAINING_PROMPT = "This workflow combines advanced Botify data downloading (Crawl, Web Logs, GSC) with URL parameter analysis, counting, visualization, and JavaScript optimization code generation. It leverages UI enhancements for debugging and file management."
    ...
    TOGGLE_CONFIG = {
        ...
        'step_05': {
            'data_key': 'parameter_counts', # Matches the 'done' key for step_05
            'status_field': 'total_unique_parameters', # A key expected in the parameter_counts JSON string
            'success_text': 'parameters counted and visualization prepared',
            'failure_text': 'parameters not yet counted or visualization not ready',
            'error_prefix': 'Error with parameter counts/visualization',
            'code_content_type': 'message', # New simple type
            'message_text': 'Parameter counting and visualization step. The primary outputs are the chart and table displayed. Key helper methods involved: `load_raw_counters_from_cache`, `calculate_and_cache_raw_counters`, `create_parameter_visualization_placeholder`.'
        }
    }
    ...
    def __init__(self, app, pipulate, pipeline, db, app_name=APP_NAME):
        ...
        crawl_template_name = self.QUERY_TEMPLATES[self.get_configured_template('crawl')]['name']
        gsc_template_name = self.QUERY_TEMPLATES[self.get_configured_template('gsc')]['name']
        
        steps = [
            Step(id='step_01', done='botify_project', show='Botify Project URL', refill=True),
            Step(id='step_02', done='analysis_selection', show=f'Download Crawl: {crawl_template_name}', refill=False),
            Step(id='step_03', done='weblogs_check', show='Download Web Logs', refill=False),
            Step(id='step_04', done='search_console_check', show=f'Download GSC: {gsc_template_name}', refill=False),
            Step(id='step_05', done='parameter_counts', show='Count Parameters & Visualize', refill=True),
            Step(id='step_06', done='parameter_optimization', show='Create Parameter Optimization JS', refill=True),
            Step(id='step_07', done='robots_txt_generation', show='View Instructions & robots.txt', refill=False)
        ]
        ...
        self.step_messages = {
            'finalize': {'ready': 'All steps complete. Ready to finalize workflow.', 'complete': f'Workflow finalized. Use {pip.UNLOCK_BUTTON_LABEL} to make changes.'},
            'step_01': {'input': f"❔{pip.fmt('step_01')}: Please enter the Botify Project URL.", 'complete': f'✳️ {pip.fmt("step_01")} Botify Project URL confirmed. Continue to next step.'},
            'step_02': {'input': f"❔{pip.fmt('step_02')}: Please select a crawl analysis for this project.", 'complete': '📊 Crawl analysis download complete. Continue to next step.'},
            'step_03': {'input': f"❔{pip.fmt('step_03')}: Please check if the project has web logs available.", 'complete': '📋 Web logs check complete. Continue to next step.'},
            'step_04': {'input': f"❔{pip.fmt('step_04')}: Please check if the project has Search Console data.", 'complete': 'Search Console check complete. Continue to next step.'},
            'step_05': {'input': f"❔{pip.fmt('step_05')}: Generate parameter counts and visualizations.", 'complete': f'📊 {pip.fmt("step_05")} Parameter counts and visualizations complete. Continue to next step.'},
            'step_06': {'input': f"❔{pip.fmt('step_06')}: Define parameters for JavaScript optimization code.", 'complete': f'⚙️ {pip.fmt("step_06")} Parameter optimization JavaScript generated. Continue to next step.'},
            'step_07': {'input': f"❔{pip.fmt('step_07')}: View instructions and generated robots.txt rules.", 'complete': f'📄 {pip.fmt("step_07")} Instructions and robots.txt ready. Ready to finalize.'}
        }
        # Fallback for any steps not explicitly defined (should not happen with current structure)
        for step_obj in steps: # Renamed step to step_obj
            if step_obj.id not in self.step_messages:
                self.step_messages[step_obj.id] = {'input': f'❔{pip.fmt(step_obj.id)}: Please complete {step_obj.show}.', 'complete': f'✳️ {step_obj.show} complete. Continue to next step.'}
...
"""
    ...
    async def common_toggle(self, request):
        ...
        # Handle simple content case (step_05 was, but now is message type)
        if config.get('code_content_type') == 'message':
            message_text = config.get('message_text', 'No specific code to display for this step.')
            content_to_display = Pre(message_text, cls='code-block-container')
        # Original 'simple_content' logic - can be removed if 'message' type covers all such cases
        # elif 'simple_content' in config: 
        #     content_to_display = Pre(config['simple_content'], cls='code-block-container')
        # Handle complex data-driven content (existing logic for python_command)
        else:
            step_index = self.steps_indices[step_id]
            step = steps[step_index]
            step_data = pip.get_step_data(pipeline_id, step_id, {})
            
            data_str = step_data.get(step.done, '') # Use step.done which matches TOGGLE_CONFIG's data_key
            data_obj = json.loads(data_str) if data_str else {}
            
            python_command = data_obj.get('python_command', '# No Python command available for this step.')
            
            status_prefix = config.get('status_prefix', '')
            if 'error' in data_obj:
                status_text = f'{config.get("error_prefix", "Error")}: {data_obj["error"]}'
                status_color = 'red'
            else:
                # Use status_field from config to check for success
                has_data = data_obj.get(config.get('status_field', 'download_complete'), False) 
                status_text = f'{status_prefix}{config.get("success_text", "Completed") if has_data else config.get("failure_text", "Not completed")}'
                status_color = 'green' if has_data else 'orange' # orange for not completed, red for error
            ...
            content_to_display = Div(
                ...
                Script(f"""
                    setTimeout(function() {{
                        if (typeof Prism !== 'undefined' && document.getElementById('{step_id}_widget')) {{
        ...
        # Visibility toggle logic remains the same
        state = pip.read_state(pipeline_id)
        widget_state_key = f'{step_id}_widget_visible'
        is_visible = state.get(widget_state_key, False)

        # If first time toggling, set to visible and show
        if widget_state_key not in state:
            state[widget_state_key] = True
            pip.write_state(pipeline_id, state)
            return content_to_display
        
        # Otherwise, toggle visibility
        state[widget_state_key] = not is_visible
        pip.write_state(pipeline_id, state)
        
        if not state[widget_state_key]: # if now hidden
            content_to_display.attrs['style'] = 'display: none;'
...
'''
    ...
    async def step_05(self, request):
        """Handles GET request for Parameter Counting and Visualization step."""
        pip, db, steps, app_name = (self.pipulate, self.db, self.steps, self.app_name)
        step_id = 'step_05'
        step_index = self.steps_indices[step_id]
        step = steps[step_index]
        ...
        # Correctly get step_data for this step (step_05)
        step_data = pip.get_step_data(pipeline_id, step_id, {})
        # The result of step_05_process (parameter_summary_str) is stored in step.done key ('parameter_counts')
        parameter_summary_str = step_data.get(step.done, '') 
        
        param_count_display_str = '40' # Default value for the slider
        if parameter_summary_str:
            try:
                summary_data = json.loads(parameter_summary_str)
                param_count_display_str = str(summary_data.get('param_count', 40))
            except json.JSONDecodeError:
                pass # Keep default if summary is not valid JSON

        # Finalized and Revert phase logic depends on parameter_summary_str
        finalize_data = pip.get_step_data(pipeline_id, 'finalize', {})

        if 'finalized' in finalize_data and parameter_summary_str:
            try:
                visualization_widget = self.create_parameter_visualization_placeholder(parameter_summary_str)
                action_buttons = self._create_action_buttons(step_data, step_id) 
                
                # Placeholder for code toggle - step_05's "code" is the visualization itself or its generation logic
                code_toggle_widget = Div(
                    ...
                    Div(
                        Pre('Parameter counting and visualization logic is complex and handled server-side. The generated chart and table are the primary outputs for this step.', cls='code-block-container', style='display: none;'),
                        id=f'{step_id}_widget'
                    ),
                    style=self.UI_CONSTANTS['BUTTON_STYLES']['FLEX_CONTAINER']
                )
                combined_widget = Div(visualization_widget, Div(action_buttons, style=self.UI_CONSTANTS['BUTTON_STYLES']['FLEX_CONTAINER']), code_toggle_widget)
                return Div(Card(H3(f'🔒 {step.show}'), combined_widget), Div(id=next_step_id, hx_get=f'/{app_name}/{next_step_id}', hx_trigger='load'), id=step_id)
            except Exception as e:
                logging.error(f'Error in finalized step_05: {str(e)}')
                return Div(Card(H3(f'🔒 {step.show}'), P('Parameter analysis completed.', cls='mb-10px')), Div(id=next_step_id, hx_get=f'/{app_name}/{next_step_id}', hx_trigger='load'), id=step_id)

        elif parameter_summary_str and state.get('_revert_target') != step_id:
            try:
                visualization_widget = self.create_parameter_visualization_placeholder(parameter_summary_str)
                action_buttons = self._create_action_buttons(step_data, step_id)
                ...
                code_toggle_widget = Div(
                    ...
                    Div(Pre('Parameter counting and visualization logic executed on server. The chart and table are the output.', cls='code-block-container', style='display: none;'),
                        id=f'{step_id}_widget'
                    )
                )
                ...
                message = f"{step.show}: {json.loads(parameter_summary_str).get('total_unique_parameters', 0):,} unique parameters found"
                return Div(pip.display_revert_widget(step_id=step_id, app_name=app_name, message=message, widget=combined_display_widget, steps=steps), Div(id=next_step_id, hx_get=f'/{app_name}/{next_step_id}', hx_trigger='load'), id=step_id)
            except Exception as e:
                logging.error(f'Error in revert step_05: {str(e)}')
                return Div(pip.display_revert_header(step_id=step_id, app_name=app_name, message=f'{step.show}: Parameter analysis complete (visualization error)', steps=steps), Div(id=next_step_id, hx_get=f'/{app_name}/{next_step_id}', hx_trigger='load'), id=step_id)

        # Input Phase
        await self.message_queue.add(pip, self.step_messages[step_id]['input'], verbatim=True)
        # Correct hx_post for the form
        form_hx_post = f'/{self.app_name}/{step_id}_submit'
        
        return Div(Card(H3(f'{step.show}'), 
                        P('This step will count URL parameters from the downloaded Crawl, Web Logs (if available), and GSC data. The results will be visualized to help identify parameters that appear frequently in crawled/logged URLs but have low or no presence in GSC data (indicating potential for optimization).', cls='mb-15px'), 
                        Form(
                            Div(
                                P("Adjust the slider to control how many parameters are shown in the generated visualization. This does not affect the underlying data counting.", style=(pip.get_style('muted') or '') + 'margin-bottom: 10px;'),
                                Label(NotStr('<strong>Number of Parameters to Display:</strong>'), For='param_count', style='min-width: 220px;'), 
                                Input(type='range', name='param_count_slider', id='param_count_slider', value=param_count_display_str, min='10', max='100', step='5', style='flex-grow: 1; margin: 0 10px;', _oninput="document.getElementById('param_count').value = this.value;"), 
                                Input(type='number', name='param_count', id='param_count', value=param_count_display_str, min='10', max='100', step='5', style='width: 100px;', _oninput="document.getElementById('param_count_slider').value = this.value;", _onkeydown="if(event.key === 'Enter') { event.preventDefault(); return false; }"), 
                                style='display: flex; align-items: center; gap: 10px; margin-bottom: 15px;'
                            ),
                            Button('Count Parameters & Visualize ▸', type='submit', cls='primary'),
                            hx_post=form_hx_post, 
                            hx_target=f'#{step_id}'
                        )
                    ), 
                    Div(id=next_step_id), 
    ...
    async def step_05_submit(self, request):
        """Process the parameter counting and visualization trigger.
        This method returns a progress bar and triggers the background processing.
        """
        pip, db, steps, app_name = (self.pipulate, self.db, self.steps, self.app_name)
        step_id = 'step_05'
        ...
        current_step_obj = steps[self.steps_indices[step_id]]

        await self.message_queue.add(pip, f"Requesting parameter counts (display top {param_count}) & visualization...", verbatim=True)

        return Card(H3(f'{current_step_obj.show}'), 
                    P('Counting parameters and preparing visualization...', cls='mb-15px'), 
                    Progress(style='margin-top: 10px;'), 
                    Script(f"""
                        setTimeout(function() {{
                            htmx.ajax('POST', '/{self.app_name}/step_05_process', {{
                                target: '#{step_id}',
                                values: {{ 
    ...
    async def step_05_process(self, request):
        ...
        try:
            ...
            await self.message_queue.add(pip, "Error: Could not load project or analysis data. Please ensure Steps 1 and 2 are complete.", verbatim=True, msg_type="error")
        ...
        if not all([username, project_name, analysis_slug]):
            await self.message_queue.add(pip, "Error: Missing required project information. Please ensure Steps 1 and 2 are complete.", verbatim=True, msg_type="error")
        ...
        try:
            ...
            if output_data is None:
                ...
                if output_data is None: 
                    await self.message_queue.add(pip, "Error: Failed to calculate parameter counts from source files.", verbatim=True, msg_type="error")
            ...
            logging.exception(f'Error in step_05_process: {e}')
            await self.message_queue.add(pip, f'❌ Error during parameter analysis: {str(e)}', verbatim=True, msg_type="error")
    ...
    async def step_05_submit(self, request):
        ...
        pipeline_id = db.get('pipeline_id', 'unknown') 
        form = await request.form()
        param_count = form.get('param_count', '40') # Default to 40 if not provided
    ...
    async def step_05_process(self, request):
        ...
        try:
            ...
            await self.message_queue.add(pip, "Error: Could not load project or analysis data from prior steps. Please ensure Steps 1 and 2 are complete.", verbatim=True, msg_type="error")
            return P('Error: Could not load project or analysis data. Please complete previous steps.', style=pip.get_style('error'))
        ...
        if not all([username, project_name, analysis_slug]):
            await self.message_queue.add(pip, "Error: Missing required project information (username, project name, or analysis slug). Please ensure Steps 1 and 2 are fully complete.", verbatim=True, msg_type="error")
            return P('Error: Missing required project information. Please complete previous steps.', style=pip.get_style('error'))

        try:
            await self.message_queue.add(pip, f'⚙️ Counting parameters for {username}/{project_name}/{analysis_slug}... This may take a few moments.', verbatim=True)
            
            data_dir_path = await self.get_deterministic_filepath(username, project_name, analysis_slug) 
            cache_filename = '_raw_param_counters_cache.pkl' 
            files_to_process = {'not_indexable': 'crawl.csv', 'gsc': 'gsc.csv', 'weblogs': 'weblog.csv'}
            
            # Corrected call to load_raw_counters_from_cache
            output_data = self.load_raw_counters_from_cache(str(data_dir_path), cache_filename) # Ensure data_dir_path is string
            
            if output_data is None:
                await self.message_queue.add(pip, 'Cache miss or invalid. Calculating parameter counts from source files. This might take longer.', verbatim=True)
                output_data = await self.calculate_and_cache_raw_counters(
                    data_directory_path=str(data_dir_path), # Ensure data_dir_path is string
                    input_files_config=files_to_process, 
                    cache_filename=cache_filename
                )
                if output_data is None: 
                    await self.message_queue.add(pip, "Error: Failed to calculate parameter counts. Source files might be missing or corrupted.", verbatim=True, msg_type="error")
                    raise ValueError('Failed to calculate counters from source files and cache is empty/invalid.')
            else:
                 await self.message_queue.add(pip, '📊 Loaded parameter counts successfully from cache.', verbatim=True)
            ...
            parameter_summary = {
                'timestamp': datetime.now().isoformat(), 
                'data_sources': {}, 
                'cache_path': str(Path(data_dir_path) / cache_filename), # Store full cache path
                'param_count': param_count_display # Store user's display preference for visualization
            }
            ...
            for source, counter_data in raw_counters.items():
                ...
                source_occurrences_val = sum(counter.values())
                total_occurrences_val += source_occurrences_val
                status = file_statuses.get(source, 'Status unknown') # Default status if not in metadata
            ...
            # --- UI Rendering part (to be fully implemented in the next step) ---
            visualization_widget = self.create_parameter_visualization_placeholder(summary_str) # Generate the viz
            current_step_data_for_ui = pip.get_step_data(pipeline_id, step_id, {}) # Re-fetch to get the saved summary_str
            action_buttons = self._create_action_buttons(current_step_data_for_ui, step_id)
            
            # Code toggle for step_05
            # The content of the toggle should explain what this step does, as the "code" is server-side logic.
            # This requires TOGGLE_CONFIG['step_05'] to be set up, likely with 'simple_content' or a custom data key.
            # For now, we assume TOGGLE_CONFIG['step_05'] will point to a python_command or descriptive text.
            # If TOGGLE_CONFIG['step_05'] is not set, this button might not fully work with common_toggle.
            # A simple placeholder for the toggle content:
            step_05_code_explanation = "The Python code for this step involves loading data from cached CSV files (Crawl, GSC, Weblogs), counting URL parameter occurrences using `collections.Counter`, and then generating the visualization table and chart using Matplotlib. Key methods: `load_raw_counters_from_cache`, `calculate_and_cache_raw_counters`, `create_parameter_visualization_placeholder`."
            
            # Store this explanation in step_data if it's to be used by common_toggle
            # For now, we'll build the UI directly.
            code_toggle_widget = Div(
                Button(self.UI_CONSTANTS['BUTTON_LABELS']['HIDE_SHOW_CODE'], 
                    cls=self.UI_CONSTANTS['BUTTON_STYLES']['STANDARD'],
                    hx_get=f'/{app_name}/toggle?step_id={step_id}', 
                ...
                Div(Pre(step_05_code_explanation, cls='code-block-container', style='display: none;'), # Content for toggle
                    id=f'{step_id}_widget'
                )
            ) # This UI structure assumes common_toggle will handle visibility.
            ...
            logging.exception(f'Error in step_05_process: {e}')
            await self.message_queue.add(pip, f'❌ Error during parameter analysis: {str(e)}', verbatim=True, msg_type="error")
            error_summary = json.dumps({'error': str(e), 'param_count': param_count_display}) # Include param_count for consistency
            await pip.set_step_data(pipeline_id, step_id, {current_step_obj.done: error_summary}, steps, is_json=False)

            error_display_widget = P(f'Error generating parameter analysis: {str(e)}', style=pip.get_style('error'))
            # Provide action buttons even on error to allow user to navigate or view existing files
            action_buttons = self._create_action_buttons(pip.get_step_data(pipeline_id, step_id, {}), step_id) 
