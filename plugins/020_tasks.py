...
class CrudUI(PluginIdentityManager):
    ...
    async def landing(self, request=None):
        ...
        return Div(Card(H2(f'{self.DISPLAY_NAME} {LIST_SUFFIX}'), Ul(*[self.app_instance.render_item(item) for item in items], id=self.LIST_ID, cls='sortable', style='padding-left: 0;'), header=Form(Group(Input(placeholder=add_placeholder, id=self.INPUT_ID, name=self.FORM_FIELD_NAME, autofocus=True), Button('Add', type='submit')), hx_post=self.ENDPOINT_PREFIX, hx_swap='beforeend', hx_target=f'#{self.LIST_ID}', hx_on__after_request="this.reset(); document.getElementById(this.querySelector('input').id).focus();")), id=self.CONTAINER_ID, style='display: flex; flex-direction: column;')
...
def render_item(item, app_instance):
    ...
    delete_icon = A('🗑', hx_delete=delete_url, hx_swap='outerHTML', hx_target=f'#{item_id}', style='cursor: pointer; display: inline; margin-left: 5px; text-decoration: none;', cls='delete-icon')
    update_input_id = f'{app_instance.name}_text_{item.id}'
    text_display = Span(item.text, id=f'{app_instance.name}-text-display-{item.id}', style='margin-left: 5px; cursor: pointer;', onclick=f"document.getElementById('{app_instance.name}-text-display-{item.id}').style.display='none'; document.getElementById('update-form-{item.id}').style.display='inline-flex'; document.getElementById('{item_id}').style.alignItems='baseline'; document.getElementById('{update_input_id}').focus();")
    update_form = Form(Group(Input(type='text', id=update_input_id, value=item.text, name=app_instance.plugin.FORM_FIELD_NAME, style='flex-grow: 1; margin-right: 5px; margin-bottom: 0;'), Button('Save', type='submit', style='margin-bottom: 0;'), Button('Cancel', type='button', style='margin-bottom: 0;', cls='secondary', onclick=f"document.getElementById('update-form-{item.id}').style.display='none'; document.getElementById('{app_instance.name}-text-display-{item.id}').style.display='inline'; document.getElementById('{item_id}').style.alignItems='center';"), cls='align-items-center'), style='display: none; margin-left: 5px;', id=f'update-form-{item.id}', hx_post=update_url, hx_target=f'#{item_id}', hx_swap='outerHTML')
    return Li(checkbox, text_display, update_form, delete_icon, id=item_id, cls='done' if item.done else '', style='list-style-type: none; display: flex; align-items: center; margin-bottom: 5px;', data_id=item.id, data_priority=item.priority, data_plugin_item='true', data_list_target=app_instance.plugin.LIST_ID, data_endpoint_prefix=app_instance.plugin.ENDPOINT_PREFIX)