...
class DropdownWidget:
    ...
    async def step_01(self, request):
        ...
        if 'finalized' in finalize_data and selected_value:
            return Div(Card(H3(f'🔒 {step.show}'), P(f'Selected: {selected_value}', cls='fw-bold')), Div(id=next_step_id, hx_get=f'/{app_name}/{next_step_id}', hx_trigger='load'), id=step_id)
        ...
        try:
            ...
            select = Select(*[Optgroup(*[Option(option['label'], value=option['value'], selected=option['value'] == selected_value, title=option['description'] if option['description'] else None) for option in group_options], label=group) for group, group_options in grouped_options.items()], name=step.done, required=True, autofocus=True)
            return Div(Card(H3(f'{step.show}'), Form(select, Button('Submit', type='submit', cls='primary'), hx_post=f'/{app_name}/{step_id}_submit', hx_target=f'#{step_id}')), Div(id=next_step_id), id=step_id)
