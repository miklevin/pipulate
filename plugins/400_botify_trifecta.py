...
"""
    ...
    def _generate_bqlv2_python_code(self, query_payload, username, project_name, page_size, jobs_payload):
        ...
        # Extract template information for header generation
        template_info = None
        qualifier_config = None
        try:
            analysis_template = self.get_configured_template('analysis')
            template_info = self.QUERY_TEMPLATES.get(analysis_template, {})
            qualifier_config = template_info.get('qualifier_config', {}) if template_info else None
        except Exception:
            pass

        # Generate header using reusable utility
        step_name = self._get_step_name_from_payload(jobs_payload)
        header_lines = self.pipulate.generate_botify_code_header(
            display_name=f"{self.DISPLAY_NAME} Workflow",
        ...
        token_loader = self.pipulate.generate_botify_token_loader()
        http_client = self.pipulate.generate_botify_http_client(
            client_name="make_query_call",
            description="Make a BQLv2 query API call to debug the export query structure."
        )
        main_executor = self.pipulate.generate_botify_main_executor(
...
'''
    ...
    def _generate_bqlv1_python_code(self, query_payload, username, project_name, jobs_payload):
        ...
        # Insert web logs-specific notes after the template info
        ui_constants = self.pipulate.get_ui_constants()
        comment_divider = ui_constants['CODE_FORMATTING']['COMMENT_DIVIDER']
        
        # Find where to insert the web logs notes (after template info, before final links)
        insert_index = -4  # Insert before the final 4 lines (blank, jupyter link, blank, docs link, divider)
        weblog_notes = [
            "#",
        ...
        # Insert the web logs-specific notes
        for i, note in enumerate(weblog_notes):
            header_lines.insert(insert_index + i, note)
            
        header_comment = "\n".join(header_lines)

        # Generate Python code using reusable utilities with BQLv1 customizations
        token_loader = self.pipulate.generate_botify_token_loader()
