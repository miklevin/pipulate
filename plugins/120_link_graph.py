...
class LinkGraphVisualizer:
    ...
    TEMPLATE_CONFIG = {'crawl': 'Link Graph Edges', 'gsc': 'GSC Performance'}
    FEATURES_CONFIG = {'enable_skip_buttons': True}
    TOGGLE_CONFIG = {'step_02': {'data_key': 'analysis_selection', 'status_field': 'download_complete', 'success_text': 'HAS link graph edge data', 'failure_text': 'does NOT have link graph edge data', 'error_prefix': 'FAILED to download link graph edges', 'status_prefix': 'Link Graph '}, 'step_02b': {'data_key': 'node_attributes', 'status_field': 'download_complete', 'success_text': 'HAS node attributes', 'failure_text': 'does NOT have node attributes', 'error_prefix': 'FAILED to download node attributes', 'status_prefix': 'Node Attributes '}, 'step_03': {'data_key': 'weblogs_check', 'status_field': 'has_logs', 'success_text': 'HAS web logs', 'failure_text': 'does NOT have web logs', 'error_prefix': 'FAILED to download web logs', 'status_prefix': 'Project '}, 'step_04': {'data_key': 'search_console_check', 'status_field': 'has_search_console', 'success_text': 'HAS Search Console data', 'failure_text': 'does NOT have Search Console data', 'error_prefix': 'FAILED to download Search Console data', 'status_prefix': 'Project '}, 'step_05': {'simple_content': 'Visualization has been prepared and is ready.'}}
    ...
    async def step_02(self, request):
        ...
        try:
            ...
            return Div(Card(H3(f'{step.show}'), P(f"Select an analysis for project '{project_name}'"), P(f'Organization: {username}', cls='text-secondary'), P(user_message, cls='text-muted', style='font-style: italic; margin-top: 10px;'), Form(Select(*dropdown_options, name='analysis_slug', required=True, autofocus=True, hx_post=f'/{app_name}/update_button_text', hx_target='#submit-button', hx_swap='outerHTML', hx_include="this, input[name='username'], input[name='project_name']"), Input(type='hidden', name='username', value=username), Input(type='hidden', name='project_name', value=project_name), Button(button_text, type='submit', cls='mt-10px primary', id='submit-button', **{'hx-on:click': 'this.setAttribute("aria-busy", "true"); this.textContent = "Processing..."'}), hx_post=f'/{app_name}/{step_id}_submit', hx_target=f'#{step_id}')), Div(id=next_step_id), id=step_id)
    ...
    async def step_02_submit(self, request):
        ...
        analysis_result = {'analysis_slug': analysis_slug, 'project': project_name, 'username': username, 'timestamp': datetime.now().isoformat(), 'download_started': True, 'export_type': active_template_details.get('export_type', 'crawl_attributes')}
        ...
        return Card(H3(f'{step.show}'), P(f"Downloading data for analysis '{analysis_slug}'..."), Progress(style='margin-top: 10px;'), Script(f"\n                setTimeout(function() {{\n                    htmx.ajax('POST', '/{app_name}/step_02_process', {{\n                        target: '#{step_id}',\n                        values: {{\n                            'analysis_slug': '{analysis_slug}',\n                            'username': '{username}',\n                            'project_name': '{project_name}'\n                        }}\n                    }});\n                }}, 500);\n            "), id=step_id)
    ...
    async def step_02b(self, request):
        ...
        if 'finalized' in finalize_data and analysis_result:
            return Div(Card(H3(f'🔒 {step.show}'), Div(P(f'Project: {project_name}', style='margin-bottom: 5px;'), P(f'Node Attributes Downloaded: {analysis_slug}', style='font-weight: bold;'), cls='custom-card-padding-bg')), Div(id=next_step_id, hx_get=f'/{app_name}/{next_step_id}', hx_trigger='load'), id=step_id)
        elif analysis_result and state.get('_revert_target') != step_id:
            action_buttons = self._create_action_buttons(analysis_result, step_id)
            widget = Div(Div(Button(self.ui['BUTTON_LABELS']['HIDE_SHOW_CODE'], cls=self.ui['BUTTON_STYLES']['STANDARD'], hx_get=f'/{app_name}/toggle?step_id={step_id}', hx_target=f'#{step_id}_widget', hx_swap='innerHTML'), *action_buttons, style=self.ui['BUTTON_STYLES']['FLEX_CONTAINER']), Div(Pre(f'Node attributes downloaded for: {analysis_slug}', cls='code-block-container', style='display: none;'), id=f'{step_id}_widget'))
            return Div(pip.display_revert_widget(step_id=step_id, app_name=app_name, message=f'{step.show}: {analysis_slug}', widget=widget, steps=steps), Div(id=next_step_id, hx_get=f'/{app_name}/{next_step_id}', hx_trigger='load'), id=step_id)
        else:
            ...
            return Div(Card(H3(f'{step.show}'), P(f"Download node attributes for analysis '{analysis_slug}'"), P(f'Organization: {username}', cls='text-secondary'), P(user_message, cls='text-muted', style='font-style: italic; margin-top: 10px;'), Form(Div(Button(button_text, type='submit', name='action', value='download', cls='mt-10px primary', **{'hx-on:click': 'this.setAttribute("aria-busy", "true"); this.textContent = "Processing..."'}), Button(self.ui['BUTTON_LABELS']['SKIP_STEP'], type='submit', name='action', value='skip', cls='mt-10px secondary outline', style=self.ui['BUTTON_STYLES']['SKIP_BUTTON_STYLE']) if self.FEATURES_CONFIG.get('enable_skip_buttons', False) else None, style=self.ui['BUTTON_STYLES']['BUTTON_ROW']), hx_post=f'/{app_name}/{step_id}_submit', hx_target=f'#{step_id}')), Div(id=next_step_id), id=step_id)
    ...
    async def step_02b_submit(self, request):
        ...
        if action == 'skip':
            ...
            skip_result = {'download_complete': False, 'skipped': True, 'skip_reason': 'User chose to skip node attributes download', 'file_path': None, 'python_command': '', 'export_type': 'crawl_attributes', 'template_used': 'Crawl Basic'}
        ...
        analysis_result = {'analysis_slug': analysis_slug, 'project': project_name, 'username': username, 'timestamp': datetime.now().isoformat(), 'download_started': True, 'export_type': 'crawl_attributes', 'template_used': 'Crawl Basic'}
        analysis_result_str = json.dumps(analysis_result)
        await pip.set_step_data(pipeline_id, step_id, analysis_result_str, steps)
        return Card(H3(f'{step.show}'), P(f"Downloading node attributes for analysis '{analysis_slug}'..."), Progress(style='margin-top: 10px;'), Script(f"\n                setTimeout(function() {{\n                    htmx.ajax('POST', '/{app_name}/step_02b_process', {{\n                        target: '#{step_id}',\n                        values: {{\n                            'analysis_slug': '{analysis_slug}',\n                            'username': '{username}',\n                            'project_name': '{project_name}'\n                        }}\n                    }});\n                }}, 500);\n            "), id=step_id)
                    ...
                    if not success:
                        ...
                        if not download_url:
                            ...
                            try:
                                ...
                                analysis_result.update({'download_complete': True, 'download_info': {'has_file': True, 'file_path': crawl_filepath, 'timestamp': file_info['created'], 'size': file_info['size'], 'cached': False, 'row_count': len(df)}})
                            except Exception as e:
    ...
    async def step_03_submit(self, request):
        ...
        return Card(H3(f'{step.show}'), P(f"Downloading Web Logs for '{project_name}'..."), Progress(style='margin-top: 10px;'), Script(f"\n                setTimeout(function() {{\n                    htmx.ajax('POST', '/{app_name}/step_03_process', {{\n                        target: '#{step_id}',\n                        values: {{\n                            'analysis_slug': '{analysis_slug}',\n                            'username': '{username}',\n                            'project_name': '{project_name}'\n                        }}\n                    }});\n                }}, 500);\n            "), id=step_id)
    ...
    async def step_04_submit(self, request):
        ...
        project_name = project_data.get('project_name', '')
        username = project_data.get('username', '')
        return Card(H3(f'{step.show}'), P(f"Downloading Search Console data for '{project_name}'..."), Progress(style='margin-top: 10px;'), Script(f"\n                setTimeout(function() {{\n                    htmx.ajax('POST', '/{app_name}/{step_id}_complete', {{\n                        target: '#{step_id}',\n                        values: {{ 'delay_complete': 'true' }}\n                    }});\n                }}, 1500);\n            "), id=step_id)
    ...
    async def step_05(self, request):
        ...
        if 'finalized' in finalize_data and viz_url:
            return Div(Card(H3(f'🔒 {step.show}'), A('Open Visualization', href=viz_url, target='_blank', role='button', cls=self.ui['BUTTON_STYLES']['PRIMARY'])), Div(id=next_step_id, hx_get=f'/{app_name}/{next_step_id}', hx_trigger='load'), id=step_id)
        elif viz_url and state.get('_revert_target') != step_id:
            widget = Div(A('🚀 Open Visualization in New Tab', href=viz_url, target='_blank', role='button', cls=self.ui['BUTTON_STYLES']['PRIMARY']))
            return Div(pip.display_revert_widget(step_id=step_id, app_name=app_name, message=f'{step.show}: Link generated!', widget=widget, steps=steps), Div(id=next_step_id, hx_get=f'/{app_name}/{next_step_id}', hx_trigger='load'), id=step_id)
        else:
            await self.message_queue.add(pip, self.step_messages[step_id]['input'], verbatim=True)
            return Div(Card(H3(f'{step.show}'), P('This step will process the downloaded data into Cosmograph-compatible CSVs and generate a visualization link.'), Form(Button('Prepare & Visualize ▸', type='submit', cls='primary', **{'hx-on:click': 'this.setAttribute("aria-busy", "true")'}), hx_post=f'/{app_name}/{step_id}_submit', hx_target=f'#{step_id}')), Div(id=next_step_id), id=step_id)
    ...
    async def step_05_submit(self, request):
        ...
        step = steps[step_index]
        pipeline_id = db.get('pipeline_id', 'unknown')
        return Card(H3(f'{step.show}'), P('Processing data and preparing visualization... This may take a moment.'), Progress(style='margin-top: 10px;'), Script(f"\n                setTimeout(function() {{\n                    htmx.ajax('POST', '/{app_name}/step_05_process', {{\n                        target: '#{step_id}',\n                        values: {{ 'pipeline_id': '{pipeline_id}' }}\n                    }});\n                }}, 500);\n            "), id=step_id)
    ...
    async def step_05_process(self, request):
        ...
        try:
            ...
            nodes_df = nodes_df.fillna({'Impressions': 0, 'Clicks': 0, 'CTR': 0, 'Avg. Position': 100, 'crawls.google.count': 0})
            ...
            visualization_result = {'visualization_url': cosmograph_url, 'links_url': links_url, 'nodes_url': nodes_url, 'edges_count': len(edges_df), 'nodes_count': len(cosmo_nodes_df), 'timestamp': datetime.now().isoformat()}
            ...
            widget = Div(P(f'Graph processed: {len(edges_df):,} edges, {len(cosmo_nodes_df):,} nodes'), A('🚀 Open Visualization in New Tab', href=cosmograph_url, target='_blank', role='button', cls='primary'), style='text-align: center; padding: 15px;')
            return Div(pip.display_revert_widget(step_id=step_id, app_name=app_name, message=f'{step.show}: Visualization link generated', widget=widget, steps=self.steps), Div(id=next_step_id, hx_get=f'/{app_name}/{next_step_id}', hx_trigger='load'), id=step_id)
        except Exception as e:
        ...
        while next_url:
            ...
            try:
                ...
                if not analyses:
                    ...
                    return all_slugs
                if is_first_page:
                    curl_cmd, python_cmd = self._generate_api_call_representations(method='GET', url=next_url, headers=headers, step_context='Step 2: Fetch Analysis List')
                    await self.pipulate.log_api_call_details(pipeline_id='fetch_analyses', step_id='analyses_list', call_description='Fetch Analysis List (First Page)', method='GET', url=next_url, headers=headers, response_status=response.status_code, response_preview=json.dumps(data), curl_command=curl_cmd, python_command=python_cmd)
                    is_first_page = False
                else:
    ...
    def create_cosmograph_url(self, data_url, meta_url=None, **params):
        ...
        default_params = {'link-spring': '.03', 'nodeColor': 'Page Type'}
    ...
    async def get_deterministic_filepath(self, username, project_name, analysis_slug, data_type=None):
        ...
        if not data_type:
            return base_dir
        filenames = {'crawl': 'crawl.csv', 'weblog': 'weblog.csv', 'gsc': 'gsc.csv', 'crawl_attributes': 'crawl.csv', 'link_graph_edges': 'link_graph.csv', 'gsc_data': 'gsc.csv'}
    ...
    def _generate_bqlv2_python_code(self, query_payload, username, project_name, page_size, jobs_payload):
        ...
        ui_constants = self.pipulate.get_ui_constants()
        comment_divider = ui_constants['CODE_FORMATTING']['COMMENT_DIVIDER']
        header_lines = [comment_divider, f'# Botify Query API Call (BQLv2 - for debugging the export query)', f'# Generated by: {self.DISPLAY_NAME} Workflow', f'# Step: {self._get_step_name_from_payload(jobs_payload)}', f'# Organization: {username}', f'# Project: {project_name}']
        try:
            crawl_template = self.get_configured_template('crawl')
            template_info = self.QUERY_TEMPLATES.get(crawl_template, {})
            if template_info:
    ...
    def _generate_bqlv1_python_code(self, query_payload, username, project_name, jobs_payload):
        ...
        ui_constants = self.pipulate.get_ui_constants()
        comment_divider = ui_constants['CODE_FORMATTING']['COMMENT_DIVIDER']
        header_lines = [comment_divider, f'# Botify Web Logs API Call (BQLv1 - /logs endpoint)', f'# Generated by: {self.DISPLAY_NAME} Workflow', f'# Step: {self._get_step_name_from_payload(jobs_payload)}', f'# Organization: {username}', f'# Project: {project_name}', f'# Date Range: {start_date} to {end_date}', '#', '# Query Template: Web Logs (Hardcoded - KISS Principle)', '# Description: Simple web logs query with consistent fields and filters', '# Export Type: weblog', '#', '# 📝 NOTE: Web logs are intentionally NOT templated for simplicity:', "# - Always same fields: ['url', 'crawls.google.count']", '# - Always same filter: crawls.google.count > 0', '# - Uses legacy BQLv1 structure (different from crawl/GSC)', '# - Different API endpoint (app.botify.com vs api.botify.com)', '#', '# 🧪 For live JupyterLab environment to experiment with queries:', '# http://localhost:8888/lab/tree/helpers/botify/botify_api.ipynb', '#', '# 📋 For copy/paste-able examples to use in JupyterLab:', '# http://localhost:5001/documentation', comment_divider]
        header_comment = '\n'.join(header_lines)
    ...
    async def step_06(self, request):
        ...
        if 'finalized' in finalize_data and current_value:
            ...
            await self.message_queue.add(pip, self.step_messages[step_id]['input'], verbatim=True)
            return Div(Card(H3(f'{step.show}'), P('This is a new placeholder step. Customize its input form as needed. Click Proceed to continue.'), Form(Input(type='hidden', name=step.done, value='Placeholder Value for Placeholder Step 6 (Edit Me)'), Button('Next ▸', type='submit', cls='primary'), hx_post=f'/{app_name}/{step_id}_submit', hx_target=f'#{step_id}')), Div(id=next_step_id), id=step_id)
    ...
    async def update_button_text(self, request):
        ...
        try:
            ...
            project_name = form.get('project_name', '').strip()
            if not all([analysis_slug, username, project_name]):
                return Button('Download Link Graph ▸', type='submit', cls='mt-10px primary', id='submit-button', **{'hx-on:click': 'this.setAttribute("aria-busy", "true"); this.textContent = "Processing..."'})
            active_crawl_template_key = self.get_configured_template('crawl')
            active_template_details = self.QUERY_TEMPLATES.get(active_crawl_template_key, {})
            export_type = active_template_details.get('export_type', 'crawl_attributes')
            button_suffix = active_template_details.get('button_label_suffix', 'Data')
