...
> This Notebook is for Botify Employees who have a Botify API Key and for the LLMs being trained to help humans with the API. Botify Customers may also find it valuable. This Notebook runs from the JupyterLab on your local machine that gets installed side-by-side with Botifython. This means it shares the same `.venv` (virtual environment) where all the `pip` installs are done. So if you can do it there (in Pipulate *Workflows*), you can do it here. 
