...
class DropdownWidget:
    ...
    APP_NAME = 'dropdown_widget'
    DISPLAY_NAME = 'Dropdown Widget'
    ENDPOINT_MESSAGE = 'Welcome to the Dropdown Widget! This is a focused environment for implementing and testing dropdown selection widgets. Use this space to prototype and refine your dropdown designs without distractions.'
    TRAINING_PROMPT = 'This is a specialized workflow for implementing and testing dropdown selection widgets. It provides a clean environment to focus on dropdown development without the complexity of a full workflow implementation.'
    SOURCE_TYPE = 'static'
    SOURCE_CONFIG = {'api_url': None, 'file_path': None, 'db_query': None, 'previous_step': None, 'value_field': 'value', 'label_field': 'label', 'group_field': 'group', 'description_field': 'description', 'options': [{'value': '1', 'label': 'Option 1', 'group': 'Group A'}, {'value': '2', 'label': 'Option 2', 'group': 'Group A'}, {'value': '3', 'label': 'Option 3', 'group': 'Group B'}]}
    ...
    def __init__(self, app, pipulate, pipeline, db, app_name=APP_NAME):
        ...
        steps = [Step(id='step_01', done='dropdown_selection', show='Dropdown Selection', refill=True, transform=lambda prev_value: prev_value.strip() if prev_value else '')]
        ...
        for step in steps:
            self.step_messages[step.id] = {'input': f'{wand.fmt(step.id)}: Please complete {step.show}.', 'complete': f'{step.show} complete. Continue to next step.'}
    ...
    async def finalize(self, request):
        ...
        if request.method == 'GET':
            if finalize_step.done in finalize_data:
                return Article(H3('Workflow is locked.'), Form(Button(wand.UNLOCK_BUTTON_LABEL, type='submit', cls='secondary outline', data_testid='dropdown-widget-unlock-button', aria_label='Unlock dropdown for editing'), hx_post=f'/{app_name}/unfinalize', hx_target=f'#{app_name}-container'), id=finalize_step.id)
            else:
                all_steps_complete = all((wand.get_step_data(pipeline_id, step.id, {}).get(step.done) for step in steps[:-1]))
                if all_steps_complete:
                    return Article(H3('All steps complete. Finalize?'), P('You can revert to any step and make changes.', cls='text-secondary'), Form(Button('Finalize 🔒', type='submit', cls='primary', data_testid='dropdown-widget-finalize-button', aria_label='Finalize dropdown workflow'), hx_post=f'/{app_name}/finalize', hx_target=f'#{app_name}-container'), id=finalize_step.id)
                else:
    ...
    async def step_01(self, request):
        ...
        try:
            ...
            select = Select(*[Optgroup(*[Option(option['label'], value=option['value'], selected=option['value'] == selected_value, title=option['description'] if option['description'] else None, data_testid='dropdown-widget-option') for option in group_options], label=group) for group, group_options in grouped_options.items()], name=step.done, required=True, autofocus=True, data_testid='dropdown-widget-select-input', aria_label='Dropdown selection widget')
            return Div(Article(H3(f'{step.show}'), Form(select, Button('Submit', type='submit', cls='primary', data_testid='dropdown-widget-next-button', aria_label='Proceed to next step'), hx_post=f'/{app_name}/{step_id}_submit', hx_target=f'#{step_id}', data_testid='dropdown-widget-form', aria_label='Dropdown selection form')), Div(id=next_step_id), id=step_id)
    ...
    async def step_01_submit(self, request):
        ...
        await wand.set_step_data(pipeline_id, step_id, value, steps)
        await self.message_queue.add(wand, self.step_messages.get(step_id, {}).get('complete', f'{step.show} complete: {value}'), verbatim=True)
        return Div(wand.display_revert_header(step_id=step_id, app_name=app_name, message=f'{step.show}: {value}', steps=steps), Div(id=next_step_id, hx_get=f'/{app_name}/{next_step_id}', hx_trigger='load'), id=step_id)
